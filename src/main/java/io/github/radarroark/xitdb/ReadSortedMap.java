...
public class ReadSortedMap implements Slotted, Iterable<ReadCursor> {
    public ReadCursor cursor;

    protected ReadSortedMap() {
    }

    public ReadSortedMap(ReadCursor cursor) {
    ...
    public ReadCursor getCursor(byte[] key) throws Exception {
        return this.cursor.readPath(new Database.PathPart[]{
            new Database.SortedMapGet(new Database.SortedMapGetValue(key))
        });
    }

    public Slot getSlot(byte[] key) throws Exception {
        return this.cursor.readPathSlot(new Database.PathPart[]{
            new Database.SortedMapGet(new Database.SortedMapGetValue(key))
        });
    }

    public ReadCursor.KeyValuePairCursor getKeyValuePair(byte[] key) throws Exception {
        var cursor = this.cursor.readPath(new Database.PathPart[]{
            new Database.SortedMapGet(new Database.SortedMapGetKVPair(key))
        });
        if (cursor == null) {
            return null;
        } else {
